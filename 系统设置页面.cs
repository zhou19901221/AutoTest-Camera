...
namespace 自动测试
{
    public partial class 系统设置页面 : Form
    {
        ...
        private void 初始化检测设置页()
        {
            var 页面 = 检测设置页;
            
            var 检测设置组 = 创建分组框("检测设置", 20, 20, 700, 200);
            int y = 25;
            var 控件列表 = new List<Control>
            {
                创建标签("检测模式：", 20, y), 创建下拉框(new[] { "自动检测", "手动检测", "半自动检测" }, 100, y-3, 150),
                创建标签("检测次数：", 300, y), 创建数字框(1, 100, 1, 380, y-3),
                创建标签("检测间隔：", 500, y), 创建数字框(100, 10000, 1, 580, y-3), 创建标签("ms", 680, y)
            };
            y += 35;
            控件列表.AddRange(new Control[]
            {
                创建复选框("自动记录结果", 20, y, true),
                创建复选框("异常自动停止", 200, y, true),
                创建复选框("声音提示", 380, y, false),
                创建复选框("保存图像", 540, y, true)
            });
            检测设置组.Controls.AddRange(控件列表.ToArray());
            页面.Controls.Add(检测设置组);
        }
